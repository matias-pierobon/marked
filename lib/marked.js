...
;(function(root) {
  ...
  var inline = {
    ...
    nolink: /^!?\[(?!\s*\])((?:\[[^\[\]]*\]|\\[\[\]]|[^\[\]])*)\](?:\[\])?/,
    strong: /^\*([^\s][\s\S]*?[^\s])\*(?!\*)|^\*([^\s])\*(?!\*)/,
    u: /^__([^\s][\s\S]*?[^\s])__(?!\_)|^__([^\s])__(?!_)/,
    em: /^_([^\s][\s\S]*?[^\s_])_(?!_)|^_([^\s_][\s\S]*?[^\s])_(?!_)|^_([^\s_])_(?!_)/,
    code: /^(`+)\s*([\s\S]*?[^`]?)\s*\1(?!`)/,
    br: /^ {2,}\n(?!\s*$)/,
    del: noop,
    text: /^[\s\S]+?(?=[\\<!\[`*]|\b_| {2,}\n|$)/
  ...
  Renderer.prototype.space = function(size) {
    var str = ""
  ...
  Parser.prototype.tok = function() {
    switch (this.token.type) {
      case "space": {
        return this.renderer.space(this.token.size)
      }
      case "hr": {
